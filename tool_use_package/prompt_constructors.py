...
def construct_prompt_from_messages(messages):
    ...
    for i, message in enumerate(messages):
        ...
        if message["role"] == "tool_inputs":
            appendage = construct_tool_inputs_message(
                message["content"], message["tool_inputs"]
            )
            if (i > 0 and messages[i - 1]["role"] == "user") or i == 0:
                constructed_prompt = f"{constructed_prompt}\n\nAssistant:{appendage}"
    ...
    for message in messages:
        ...
        if message["role"] == "tool_outputs":
            if "content" in message:
                raise ValueError(
                    "tool_outputs should not have a 'content' key/value pair"
                )
            if (
                message["tool_error"] is not None
                and message["tool_outputs"] is not None
            ):
                raise ValueError(
            ...
            if message["tool_error"] is not None and not isinstance(
                message["tool_error"], str
            ):
                raise ValueError("tool_error must be str or None.")
...
def construct_tool_inputs_message(content, tool_inputs):
    def format_parameters(tool_arguments):
        return "\n".join(
            [f"<{key}>{value}</{key}>" for key, value in tool_arguments.items()]
        )
